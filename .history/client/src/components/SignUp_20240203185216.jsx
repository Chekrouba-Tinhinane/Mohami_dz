...
const SignupForm = ({ onCancel, onSubmit, specialities }) => {
  ...
  const handleSubmit = async (values) => {
    try {
      const endpoint = values.role === "Avocat" ? "avocat/register_avocat" : "client/register_client";
      ...
      if (values.role === "Avocat") {
        postData.avocat.first_name = values.first_name;
        postData.avocat.last_name = values.last_name;
        postData.avocat.ville = values.ville;
        postData.avocat.region = values.region;
        postData.avocat.codepostal = values.codepostal;
        postData.avocat.siteweb = values.siteweb;
        postData.avocat.langue = values.langue;
        postData.avocat.photo = values.photo;
      }

      const response = await axios.post(`http://192.168.137.210:8000/${endpoint}`, postData);
    ...
    <div className="mx-auto w-[65%] phone:max-tablet:w-full">
      ...
      <Formik
        ...
        validationSchema={Yup.object({
          username: Yup.string().required(t("validation.firstNameRequired")),
