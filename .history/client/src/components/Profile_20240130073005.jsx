...
const Profile = ({ lawyers }) => {
  ...
  return (
    <>
      ...
      <motion.div
            className={ ` ${isModalOpen ? " scale-100" : " scale-50"} transition-all duration-300 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-md z-50 w-[700px] h-[436px]` }
...
function CalendarModal({ lawyer, isOpen, onRequestClose }) {
  ...
  return (
    <Modal
      ...
      className={ ` ${isOpen ? " scale-100" : " scale-50"} transition-all duration-300 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-md z-50 w-[700px] h-[436px]` }
      overlayClassName="fixed top-0 left-0 right-0 bottom-0 bg-gray-900 bg-opacity-50 backdrop-blur-xs z-40"
    >
      <div className={`flex h-full ${showCalendar ? "gap-[1rem]" : ""}`}>
        <div
          className={`  ${
