...
function Avis({ lawyer }) {
  ...
  const submitFeedback = async (feedbackData) => {
    try {
      ...
      const response = await axios.post('http://192.168.137.210:8000/ratings/avocat_rating?id=${lawyer.avocat.id}', feedbackData);
  ...
  return (
    ...
    >
      <div className="flex justify-between font-semibold tracking-wide text-lightTypo">
        Avis
        <GiveFeedBack isOpen={isModalOpen} onSubmit={submitFeedback} onClose={closeModal} /> {/* Pass the necessary props */}
      </div>
...
const GiveFeedBack = ({ isOpen, onSubmit, onClose }) => {
