...
const LawyerCard = ({ admin, lawyer, key }) => {
  ...
  const handleApproving =() => {
    axios
    .get(
      `http://192.168.137.210:8000/avocat/verify`,
      ...
      <div className=" flex flex-col basis-[70%] gap-4 my-6">
        <div className=" basis-[45%] border-b border-b-lightTypo flex justify-between">
          ...
          <div className=" flex flex-col gap-5">
            ...
            {admin && (
              <button onClick={handleApproving} className=" hover:bg-lightTypo hover:text-white font-medium w-full h-max px-6 py-1 text-base border border-primary rounded-sm  text-primary">
                {" "}
                Approver Avocat{" "}
              </button>
            )}
