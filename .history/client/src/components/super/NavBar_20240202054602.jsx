...
const NavBar = ({ links, landing, l }) => {
  ...
  return (
    <nav
      ...
      className={`  ${
        landing ? "px-20" : ""
      } relative flex items-center pt-6 mb-3 w-full justify-between `}
    >
      {isMobile ? (
        <div className="z-70 shadow-md w-full">
          <Hamburger isOpen={isMenuOpen} onClick={handleMenuClick} />
          <div
            className={` ${
              isMenuOpen ? "h-screen" : "h-0"
            }  transition-all transform duration-200 absolute top-0 bg-red-200 `}
          >
            Open
          </div>
        </div>
