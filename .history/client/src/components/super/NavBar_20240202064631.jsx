    ...
    >
      {isMobile ? (
        <div className="relative w-screen flex">
          ...
          <div
            className={` ${
              isMenuOpen ? " opacity-95 max-h-screen z-20" : "opacity-0 -z-10"
            } flex flex-col items-center w-full absolute  shadow-md transition-height transfom duration-200 top-5 -left-0 bg-white py-3 px-3`}
          >
                ...
                <ul className="w-min ">
                  {links.map((link, index) => (
                    <motion.li
                      ...
                      whileHover={{ scale: 1, originX: 0, color: "#D4AD6B" }}
                      transition={{ type: "tween", duration: 0.2 }}
                      className="font-semibold min-w-max flex-shrink-0 opacity-80 tracking-wider cursor-pointer"
                    >
                      {l ? (
                        <ScrollLink
                          to={link.id}
