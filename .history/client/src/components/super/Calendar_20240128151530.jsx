...
export const Calendar = ({ lawyer }) => {
  ...
  const [selectedInterval, setSelectedInterval] = useState(null); // Newly added state to store the selected interval
  ...
  const getTimes = () => {
    ...
    const { justDate } = date;
    const selectedDayIntervals = availabilityIntervals.filter(
      (interval) => format(interval.DateInterval, "yyyy-MM-dd") === format(justDate, "yyyy-MM-dd")
    );
  
    if (selectedDayIntervals.length === 0) {
      return []; // No available slots for this date
    }
  
    // Combine the start times from all intervals for the selected date
    const availableTimes = selectedDayIntervals.map((interval) => interval.HeureDebut);
  ...
  const handleDateClick = (selectedDate) => {
    ...
    setActiveTime(null);
    setSelectedInterval(null); // Reset selected interval when date is clicked
  ...
  const handleTimeRowClick = (selectedTime) => {
    ...
    const selectedInterval = availabilityIntervals.find(
      (interval) =>
        format(interval.DateInterval, "yyyy-MM-dd") ===
          format(date.justDate, "yyyy-MM-dd") &&
        format(interval.HeureDebut, "kk:mm") === format(selectedTime, "kk:mm")
    );
    setSelectedInterval(selectedInterval); // Store the selected interval
  };
  ...
  const handleBooking = async () => {
    if (!selectedInterval) {
      console.error("No interval selected for booking.");
      return;
    }
    ...
    const bookingData = {
      ...
      id_interval_libre: selectedInterval.id, // Use the ID of the selected interval
    };
