...
const TimeRow = ({ index, style, data, activeTime, onClick }) => {
  const startTime = data[index].start; // Assuming data[index] is an object with 'start' and 'end' properties representing the start and end times of the time slot
  const endTime = data[index].end;

  let isActive = false;

  if (activeTime) {
    // Check if activeTime falls within the current time slot
    if (
      isAfter(activeTime.startTime, startTime) &&
      isAfter(endTime, activeTime.endTime)
    ) {
      isActive = true;
    }
  }
