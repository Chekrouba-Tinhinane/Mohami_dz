...
function Avis({ lawyer }) {
  ...
  const submitFeedback = async (feedbackData) => {
    try {
      // Assuming you have an endpoint to submit feedback data
      const response = await axios.post('http://192.168.137.210:8000/ratings/register_rating', feedbackData);
  ...
  return (
    ...
    >
      <div className="flex justify-between font-semibold tracking-wide text-lightTypo">
        Avis
        <GiveFeedBack onOpen={openModal} isOpen={isModalOpen} onSubmit={submitFeedback} onClose={closeModal} /> {/* Pass the necessary props */}
      </div>
...
const GiveFeedBack = ({ onOpen, isOpen, onSubmit, onClose }) => {
  ...
  const handleSubmit = () => {
    ...
    console.log(userData?.userID, avocat_id: lawyer?.avocat.id , comment: feedback, rating: rating)
    onSubmit({ client_id: userData?.userID, avocat_id: lawyer?.avocat.id , comment: feedback, rating: rating }); // Pass feedback and rating to parent component
