...
const LawyerCard = ({ admin, lawyer, key }) => {
  ...
  const handleDelete = () => {
    axios
      .post(`http://192.168.137.210:8000/avocat/avocat_delete`, lawyer?.avocat?.id)
      .then((response) => {
        console.log(response.data);
      })
      .catch((error) => {
        ...
        <div className=" basis-[45%] border-b border-b-lightTypo flex justify-between">
          ...
          <div className=" flex flex-col gap-5">
            <Link to={`/profile/${lawyer?.avocat?.id}`}>
              <button className=" hover:bg-opacity-70 w-full h-max px-6 py-1 text-base bg-primary text-red-">
                Voir Profil
              </button>
            </Link>
            {admin && (
              ...
              <button
                ...
                className=" hover:bg-opacity-60 hover:text-white font-medium w-full h-max px-6 py-1 text-base border border-red-400 rounded-sm  text-red-400"
              >
