...
const Profile = ({ lawyers }) => {
  ...
  return (
    <>
      ...
      <motion.div
      className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-8 rounded-md text-yellow-300 z-50 w-[700px] h-[436px] ${isModalOpen ? "scale-100" : "scale-0"} transition-all duration-500`}
    >
      <CalendarModal
        lawyer={lawyer}
        isOpen={isModalOpen}
...
function CalendarModal({ lawyer, isOpen, onRequestClose }) {
  ...
  return (
    <Modal
      ...
      className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-md z-50 w-[700px] h-[436px]  "
