    ...
    return (
      <div className="flex justify-center">
        ...
        {pages.map((page, index) => (
          <span key={index} onClick={() => handleClick(page)} style={{ cursor: "pointer", margin: "0 5px" }}>
            {page}
          </span>
        ))}
        <button onClick={() => handleClick(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages}>
          {">"}
        </button>
      </div>
    );
...
const LawyerList = ({ lawyers, currentPage, itemsPerPage }) => {
  ...
  return (
    <div className=" flex flex-col gap-8 px-">
