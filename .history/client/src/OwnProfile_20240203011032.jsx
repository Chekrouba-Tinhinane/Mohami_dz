...
const OwnProfile = ({ lawyer }) => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        setPendingRequests([{id: 1, client:  { email: "HAHAA" } } , {id: 2, client:  { email: "second" } }])
      }
    };
  ...
  const handleArchive = (appointmentId) => {
    ...
    prevRequests.filter((request) => request.id !== appointmentId)
  );
      ...
      <div className="flex flex-col gap-[4rem] mx-[4rem] my-4 py-8 px-12 bg-lightBrown min-h-max relative">
        ...
        <div className="flex flex-col gap-5">
          ...
          {pendingRequests?.length > 0 ? (
            pendingRequests?reverseEasing.map((request, index) => (
              <div
                key={index}
              ...
              >
                ...
                <button onClick={handleArchive} className="bg-primary text-white px-4 py-2 h-max w-max rounded-md hover:opacity-80">
                  {t("Accept")}
                </button>
              </div>
            ))
