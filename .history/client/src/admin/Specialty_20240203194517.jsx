...
const Specialty = () => {
  ...
  const { data: specialities, isLoading, error } = useQuery("specialities", async () => {
    const response = await axios.get("http://192.168.137.210:8000/speciality/speciality_list");
  ...
  const handleAddSpecialite = () => {
    ...
    // You can add further logic here, like sending a request to the 192.168.137.210 to save the specialite data
  ...
  return (
    <div className="flex flex-col w-full pt-5 ">
      ...
      <div className=" min-h-screen flex flex-col bg-lightBrown px-[3rem] py-[1.5rem] mx-[4rem]">
          {specialities?.map((spec, id) =>  <SpecialtyCard spec={spec} />)}
      </div>
      <Footer />
    </div>
  );
