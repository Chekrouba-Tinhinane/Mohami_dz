...
const Specialty = () => {
  ...
  const updateSpecialtyMutation = useMutation(
    (updatedSpecialty) =>
      axios.post(
        "http://192.168.137.210:8000/speciality/speciality_update",
        updatedSpecialty
      ),
    {
      onSuccess: () => {
  ...
  const deleteSpecialtyMutation = useMutation(
    (specialtyId) =>
      axios.delete(
        `http://192.168.137.210:8000/speciality/speciality_delete/${specialtyId}`
      ),
    {
      onSuccess: () => {
        refetch();
...
function SpecialtyCard({ spec, onUpdate, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-md p-4 my-2 flex justify-between items-center">
      ...
      <div>
        <button className="text-blue-500 mr-2" onClick={onUpdate}>
          Update
        </button>
