    ...
    const fetchProfileData = async () => {
      try {
        ...
        const pendingRequestsResponse = await axios.post(
          ...
          { avocatId: lawyer?.avocat?.id } // Corrected the request body format
        );
        setPendingRequests(pendingRequestsResponse.data);
      } catch (error) {
        console.error("Error fetching profile data:", error);
        // If there's an error, set some dummy data for testing purposes
        setPendingRequests([
  ...
  return (
    <>
      <div className="flex flex-col gap-[4rem] mx-[4rem] my-4 py-8 px-12 bg-lightBrown min-h-max relative">
        ...
        <div className="border-b border-gray-300 pb-4">
          ...
          <div className="space-y-4">
            {ratings.length > 0 ? (
              ratings.map((rating, index) => <Comment key={index} comment={rating} />) // Add key prop
            ) : (
        ...
        <div className="flex flex-col gap-5">
          ...
          {pendingRequests.length > 0 ? (
            pendingRequests.reverse().map((request) => (
              <div key={request.id} className="flex bg-white px-5 py-4 gap-4 justify-between rounded-md">
                <div className="space-y-2 w-full">
                  ...
                  <p className="flex items-center gap-2">
                    ...
                    {request?.timing?.HeureDebut || "a"} - {request?.timing?.HeureFin || "1"}
                  </p>
                </div>
                <button onClick={() => handleArchive(request.id)} className="bg-primary text-white px-4 py-2 h-max w-max rounded-md hover:opacity-80">
                  {t("Accept")}
                </button>
              </div>
            ))
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleConfirmArchive} className="bg-primary" variant="contained">
            {t("Yes")}
          </Button>
...
const OwnProfile = ({ lawyer }) => {
  ...
  const handleConfirmArchive = async () => {
    try {
      ...
      console.log("Appointment archived:", appointmentToArchive);
      // Remove the archived appointment from pendingRequests
      setDummyPendingRequests((prevRequests) =>
        prevRequests.filter((request) => request.id !== appointmentToArchive)
      );
      // Close the confirmation dialog
      setShowConfirmationDialog(false);
      ...
      <div className="flex flex-col gap-[4rem] mx-[4rem] my-4 py-8 px-12 bg-lightBrown min-h-max relative">
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* requestsLoading ? (
            ...
            pendingRequests
              .reverse()
              .map((request) => <PendingRequest request={request} />)
          ) :  *dummyPendingRequests ? (
            dummyPendingRequests
              .reverse()
              .map((request) => <PendingRequest onArchive={handleArchive} request={request} />)
          ) : (
            <p>{t("No pending requests at the moment.")}</p>
          )}
        </div>
